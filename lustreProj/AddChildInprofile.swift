...
class AddChildInprofile: UIViewController , UIPickerViewDelegate , UIPickerViewDataSource {
static var page = 0
func numberOfComponents(in pickerView: UIPickerView) -> Int {
      ...
      var selection : String?
        ...
        override func viewDidLoad() {
           ...
           // here if he clicks on add
            Add.addTarget(self, action: #selector(FillChildInfo), for: .touchUpInside)
 ...
 @objc func FillChildInfo(){
  ...
  ChildInfo.counter = ChildInfo.counter+1
    ...
    for n in 0...childArray.arrayCount {
    print(" print the childreen ")
        print(childArray.childreen[n]?.name )
        print(childArray.childreen[n]?.gender )
    }
    
    print("innnnn child add")
        
        
          for n in childArray.arrayCount...childArray.countChildreen-1 {
              let child = childArray.childreen[childArray.arrayCount]
              if child == nil {return }
    ...
    @IBAction func whichPage(_ sender: Any) {
//        from profile
        if AddChildInprofile.page == 0 {
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
            let secondViewController = storyboard.instantiateViewController(identifier: "profilePage") as UIViewController
            self.navigationController?.pushViewController(secondViewController, animated: true)
            
        }
//        from children
        else if AddChildInprofile.page == 1 {
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
